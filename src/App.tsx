        ...
        <>
          ...
          <footer className="relative p-2 md:p-6 bg-gradient-to-t from-black via-black/80 to-transparent z-50">
            <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3 md:gap-6">
              <div className="flex items-center gap-4 bg-black/80 backdrop-blur-3xl p-2 md:p-4 rounded-2xl border border-white/10 w-full md:w-auto justify-center md:justify-start shadow-2xl">
                <div className="flex flex-col items-center md:items-start">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm md:text-2xl font-black tracking-tighter text-white">{currentPlayer?.chips.toLocaleString() || 0}</span>
                  </div>
                </div>
              </div>

              <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto justify-center flex-wrap">
                {isMyTurn && gameState?.gameStarted && !gameState.winner && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 md:gap-3 w-full justify-center flex-wrap">
                    <button onClick={() => takeAction('fold')} className="bg-zinc-900/90 border border-white/10 text-white font-black px-4 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl text-[10px] md:text-sm uppercase tracking-widest hover:bg-zinc-800 transition-all active:scale-95 shadow-xl">Fold</button>
                    {currentPlayer?.isBlind && (
                      <button onClick={() => takeAction('see')} className="bg-zinc-900/90 border border-white/10 text-white font-black px-4 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl flex items-center gap-2 text-[10px] md:text-sm uppercase tracking-widest hover:bg-zinc-800 transition-all active:scale-95 shadow-xl">
                        <Eye className="w-4 h-4 md:w-6 md:h-6 text-red-500" /> See
                      </button>
                    )}
                    {canSideShow && <button onClick={handleSideShow} className="bg-zinc-900/90 border border-white/10 text-white font-black px-4 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl text-[10px] md:text-sm uppercase tracking-widest hover:bg-zinc-800 transition-all active:scale-95 shadow-xl">Side</button>}
                    <div className="flex items-stretch gap-px shadow-2xl rounded-xl md:rounded-2xl overflow-hidden">
                      <button onClick={() => takeAction('chaal')} className="bg-red-600 text-white font-black px-6 md:px-12 py-3 md:py-4 uppercase tracking-widest min-w-[100px] md:min-w-[180px] hover:bg-red-500 transition-all active:scale-95">
                        <div className="flex flex-col items-center">
                          <span className="text-[8px] md:text-[10px] font-black text-white/60 leading-none mb-1">CHAAL</span>
                          <span className="text-sm md:text-2xl leading-none">{(currentPlayer?.isBlind ? gameState?.lastBet : (gameState?.lastBet || 0) * 2)?.toLocaleString()}</span>
                        </div>
                      </button>
                      <button onClick={handleRaise} className="bg-red-800 text-white font-black px-4 md:px-8 rounded-r-none border-l border-red-500/30 text-lg md:text-3xl hover:bg-red-700 transition-all active:scale-95 flex items-center justify-center">+</button>
                    </div>
                  </motion.div>
                )}
              </div>
